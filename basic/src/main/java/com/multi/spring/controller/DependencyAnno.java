...
public class DependencyAnno extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * 어노테이션을 활용한 객체 의존성 주입 어노테이션이 붙은 클래스를 탐색해서 컨테이너에 자동으로 등록(컴포넌트 스캔)
		 * 
		...
		/*
		 * AbstractApplicationContext cntx = new
		 * AnnotationConfigApplicationContext(BeanFactory.class);
		 * 
		 * Person2 p = (Person2)cntx.getBean("createPerson"); p.getMyFood().eat("태리");
		 * System.out.println(p);
		 * 
		 * Person2 p2 = (Person2)cntx.getBean("createPerson2");
		 * p.getMyFood().eat("푸라닭"); System.out.println(p2);
		 */
		 ...
		 * [com.multi.spring.di.model.anno.dto.Person] 같은이름의 클래스 2개일떄는 getBean사용못함
		 */
